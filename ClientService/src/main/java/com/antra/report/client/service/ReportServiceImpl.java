...
//        this.emailService = emailService;
    ...
    private void sendDirectRequests(ReportRequest request) {
        ...
        // change origianl response to future we just declared and get the response
        try {
            Future<ExcelResponse>  excelResponseFuture = executorService.submit(() -> rs.postForEntity("http://localhost:8888/excel", request, ExcelResponse.class).getBody());
        ...
        try {
            Future<PDFResponse> pdfResponseFuture = executorService.submit(() -> rs.postForEntity("http://localhost:9999/pdf", request, PDFResponse.class).getBody());
